...
const PLAN_LABELS: Record<string, string> = {
  free: "DropHouse — Free trial",
  "self-serve": "DropHouse — Self-serve plan",
  team: "DropHouse — Team plan",
  concierge: "DropHouse — Concierge setup",
};
...
export async function POST(request: Request) {
  ...
  const orderId = `drophouse-${plan}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  const payload = {
    ...
    order_id: orderId,
    order_description: PLAN_LABELS[plan] ?? `DropHouse — ${plan}`,
